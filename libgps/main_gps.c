...
static void start_gps ()
{
    ...
    if (state.ctrl_state == ST_STOPPED ||
        ...
        state.fd = open("/dev/ttySAC1", O_RDONLY); 
...
static int x_gps_init(GpsCallbacks * callbacks)
{   
    int ret;
    ...
    state.main_thread = state.create_thread_callback("x_main_thread",
		    main_loop, NULL);
...
static void x_gps_cleanup(void)
{
    int i;
    D("%s: enter", __FUNCTION__);
...
static int
x_gps_inject_time(GpsUtcTime time, int64_t timeReference,
		    int uncertainty)
{
    char buff[100];
    int tow, week;
 ...
 */
static int
x_gps_inject_location(double latitude, double longitude, float accuracy)
{
...
static void x_gps_delete_aiding_data(GpsAidingData flags)
{
...
static int x_gps_set_position_mode(GpsPositionMode mode,
...
static const void *x_gps_get_extension(const char *name)
{
...
const GpsInterface* x_get_gps_interface(struct gps_device_t* dev)
{
    ALOGD("x_gps_get_hardware_interface");
    return &mbmGpsInterface;
}

static int x_open_gps(const struct hw_module_t* module,
			char const* name,
			struct hw_device_t** device)
{
  struct gps_device_t *dev;
    ...
    dev->get_gps_interface = x_get_gps_interface;
...
static struct hw_module_methods_t x_gps_module_methods = {
    .open = x_open_gps
};
